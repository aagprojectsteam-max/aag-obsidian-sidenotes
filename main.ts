...
interface SideNote {
  id: string;
  text: string;
  type?: "text" | "image" | "audio" | "video" | "file" | "url";
  imagePath?: string;
  imageName?: string;
  imageMime?: string;
  imageZoom?: number;
  imageFrameHeightPx?: number;
  imageFrameScale?: number;
...
interface SideNotesScrollState {
  anchorNoteKey?: string;
  contentAnchorTop?: number;
  contentTop: number;
  listAnchorTop?: number;
  listTop: number;
  viewContentAnchorTop?: number;
...
interface SideNotesViewStats {
  fileSelectionGroups?: SideNoteReference[][];
  fileCount?: number;
  noteCount: number;
  orphanedFileCount?: number;
}

interface DraftUndoState {
  value: string;
...
interface SideNotesImportAttachmentsResult {
  attachmentCount: number;
...
export default class SideNotesPlugin extends Plugin {
  ...
  private suppressContextRefreshUntil = 0;

  private importInProgress = false;
  private importCreated: TFile[] | null = null;
  private storeSaveQueue: Promise<void> = Promise.resolve();

  acquireStoreOwnership() {
    const host = this.app as unknown as Record<symbol, unknown>;
    const installed = (this.app as unknown as { plugins?: { plugins?: Record<string, unknown> } }).plugins?.plugins ?? {};
    for (const id of ["context-aware-paragraph-notes", "aag-sidenotes"]) {
      if (installed[id] && installed[id] !== this) throw new Error("Disable the other SideNotes installation before enabling this plugin.");
    }
    if (host[SIDE_NOTES_OWNER] && host[SIDE_NOTES_OWNER] !== this) throw new Error("Another SideNotes instance owns this vault store.");
  ...
  assertStoreOwnership() {
    const installed = (this.app as unknown as { plugins?: { plugins?: Record<string, unknown> } }).plugins?.plugins ?? {};
    for (const id of ["context-aware-paragraph-notes", "aag-sidenotes"]) {
      if (installed[id] && installed[id] !== this) throw new Error("Disable the other SideNotes installation before writing notes.");
    }
    const host = this.app as unknown as Record<symbol, unknown>;
    if (host[SIDE_NOTES_OWNER] && host[SIDE_NOTES_OWNER] !== this) throw new Error("Another SideNotes instance owns this vault store.");
  }
  ...
  onunload() {
    const host = this.app as unknown as Record<symbol, unknown>;
    if (host[SIDE_NOTES_OWNER] === this) delete host[SIDE_NOTES_OWNER];
  ...
  private handleActiveLeafChange(leaf: WorkspaceLeaf | null) {
    ...
    if (this.currentContext?.filePath === nextFile.path) {
  ...
  async saveSideNotesData(importCommit = false) {
    this.assertStoreOwnership();
    if (this.importInProgress && !importCommit) throw new Error("Store save paused during import.");
    const serialized = JSON.stringify(this.sideNotesData, null, 2);
    const run = this.storeSaveQueue.then(async () => {
      this.assertStoreOwnership();
      if (!importCommit && await this.app.vault.adapter.exists(IMPORT_JOURNAL)) throw new Error("Recover the interrupted import before saving.");
  ...
  refreshContext(options: RefreshContextOptions = {}) {
  ...
  private isContextRefreshSuppressed(): boolean {
  ...
  focusLastMarkdownEditor() {
    const markdownView = this.app.workspace.getActiveViewOfType(MarkdownView) ?? this.lastMarkdownView;
  ...
  async createTransferFileForOrphanedFile(storedFile: StoredFileNotes): Promise<SideNotesTransferFile> {
  ...
  async getTransferAttachmentsForFile(file: TFile, content: string, blocks: Record<string, BlockNotes> = {}): Promise<SideNotesTransferAttachment[]> {
  ...
  async getTransferAttachmentsForSideNoteMedia(blocks: Record<string, BlockNotes>): Promise<SideNotesTransferAttachment[]> {
    return this.getTransferAttachmentsForFiles(this.getSideNoteMediaFiles(blocks));
  }

  async getTransferAttachmentsForFiles(attachmentFiles: TFile[]): Promise<SideNotesTransferAttachment[]> {
  ...
  getSideNoteMediaFiles(blocks: Record<string, BlockNotes>): TFile[] {
    const files: TFile[] = [];
    const seenPaths = new Set<string>();

    for (const blockNotes of Object.values(blocks)) {
      for (const note of blockNotes.notes ?? []) {
  ...
  async getAvailableSideNotesBundlePath(files: TFile[], suggestedName = this.getSideNotesBundleFileName(files)): Promise<string> {
  ...
  async importSideNotesTransferFile(transferFile: SideNotesTransferFile, importedAttachmentPaths = new Map<string, string>()): Promise<{ noteCount: number; attachmentCount: number }> {
    ...
    this.importCreated?.push(importedFile);
  ...
  async importSideNotesTransferAttachments(attachments: SideNotesTransferAttachment[], sideNoteMediaPaths = new Set<string>(), importedAttachmentPaths = new Map<string, string>()): Promise<SideNotesImportAttachmentsResult> {
    ...
    for (const attachment of attachments) {
      ...
      try {
        ...
        this.importCreated?.push(created);
  ...
  async copySelectedNotesToClipboard(noteRefs: SideNoteReference[]) {
    if (noteRefs.length === 0) {
      new Notice("Select one or more side notes to copy.");
      return;
    }
    ...
    if (notes.length === 0) {
      new Notice("Could not find selected side notes to copy.");
      return;
    }
    ...
    try {
      ...
      new Notice(`${notes.length} note${notes.length === 1 ? "" : "s"} copied.`);
    } catch (error) {
      console.error(error);
      if (noteTexts.length > 0) {
        try {
          await copyTextToClipboard(noteTexts.join("\n\n"));
          new Notice("Text copied, but the media could not be copied.");
          return;
        } catch (fallbackError) {
      ...
      new Notice("Could not copy notes to clipboard.");
    }
  }
  ...
  async openSideNoteFile(note: SideNote) {
    try {
      ...
      new Notice("Could not find a file path for this note.");
    } catch (error) {
      console.error(error);
  ...
  async openSideNoteUrl(note: SideNote) {
    if (!isUrlNote(note)) {
      new Notice("Could not find a URL for this note.");
      return;
    }
  ...
  getFullVaultFilePath(path: string): string {
  ...
  async addImageNoteForCurrentContext(file: File, refreshViews = true): Promise<boolean> {
    if (!isAcceptedImageFile(file.name, file.type)) {
      new Notice("Choose an image file.");
      return false;
    }
  ...
  async addAudioNoteForCurrentContext(file: File, refreshViews = true): Promise<boolean> {
    const context = await this.ensureCurrentContextHasBlockId();
    if (!context?.blockId) {
      return false;
    }
  ...
  async addAudioNoteForContext(file: File, context: ParagraphContext, refreshViews = true): Promise<boolean> {
    if (!isAcceptedAudioFile(file.name, file.type)) {
      new Notice("Choose an audio file.");
      return false;
    }
  ...
  async addVideoNoteForCurrentContext(file: File, refreshViews = true): Promise<boolean> {
    if (!isAcceptedVideoFile(file.name, file.type)) {
      new Notice("Choose a video file.");
      return false;
    }
  ...
  async addFileNoteForCurrentContext(file: File, refreshViews = true): Promise<boolean> {
    const context = await this.ensureCurrentContextHasBlockId();
    if (!context?.blockId) {
      return false;
    }
  ...
  async addFilePathNoteForCurrentContext(path: string, refreshViews = true): Promise<boolean> {
    const trimmedPath = path.trim();
    if (!trimmedPath) {
      new Notice("Enter a file path first.");
      return false;
    }
  ...
  async addUrlNoteForCurrentContext(rawUrl: string, refreshViews = true): Promise<boolean> {
    const url = normalizeSideNoteUrl(rawUrl);
    if (!url) {
      new Notice("Enter a valid URL first.");
      return false;
    }
  ...
  async updateNote(blockId: string, noteId: string, text: string, sourcePath?: string, sourceSideNoteId?: string, renderOptions: RenderOptions = {}) {
    const context = this.currentContext;
  ...
  async updateImageNoteZoom(blockId: string, noteId: string, zoom: number, sourcePath?: string, sourceSideNoteId?: string, renderOptions: RenderOptions = {}) {
    const context = this.currentContext;
    const filePath = sourcePath ?? context?.filePath ?? this.lastMarkdownView?.file?.path;
    if (!filePath && !sourceSideNoteId) {
      return;
  ...
  async updateImageNoteViewport(
    blockId: string,
    noteId: string,
    zoom: number,
    frameHeightPx: number | undefined,
  ...
  async updateImageNoteRotation(blockId: string, noteId: string, rotation: number, sourcePath?: string, sourceSideNoteId?: string, renderOptions: RenderOptions = {}) {
    const context = this.currentContext;
    const filePath = sourcePath ?? context?.filePath ?? this.lastMarkdownView?.file?.path;
    if (!filePath && !sourceSideNoteId) {
      return;
  ...
  async updateImageNotePan(blockId: string, noteId: string, panX: number, panY: number, sourcePath?: string, sourceSideNoteId?: string, renderOptions: RenderOptions = {}) {
    const context = this.currentContext;
    const filePath = sourcePath ?? context?.filePath ?? this.lastMarkdownView?.file?.path;
    if (!filePath && !sourceSideNoteId) {
      return;
  ...
  async moveNoteWithinBlock(blockId: string, noteId: string, direction: -1 | 1, sourcePath?: string, sourceSideNoteId?: string): Promise<boolean> {
    const context = this.currentContext;
    const filePath = sourcePath ?? context?.filePath ?? this.lastMarkdownView?.file?.path;
    if (!filePath && !sourceSideNoteId) {
      return false;
  ...
  async mergeSelectedNotesWithinBlock(noteRefs: SideNoteReference[]): Promise<number> {
...
class SideNotesView extends ItemView {
  ...
  private viewControlsEl: HTMLElement | null = null;
  private viewControlNotes: SideNote[] = [];
  private viewControlSelectableNoteRefs: SideNoteReference[] = [];
  private viewControlStats: SideNotesViewStats = { noteCount: 0 };
  private headerFileStatsEl: HTMLElement | null = null;
  private headerNoteStatsEl: HTMLElement | null = null;
  private shouldFocusComposer = false;
  private audioRecordingPanelVisible = false;
  private audioRecordingCancelButton: ButtonComponent | null = null;
  private audioRecordingChunks: Blob[] = [];
  private audioRecordingContext: ParagraphContext | null = null;
  private audioRecordingElapsedEl: HTMLElement | null = null;
  private audioRecordingMediaRecorder: MediaRecorder | null = null;
  private audioRecordingSaving = false;
  private audioRecordingShouldSaveOnStop = false;
  private audioRecordingStartButton: ButtonComponent | null = null;
  private audioRecordingStartedAt = 0;
  private audioRecordingStatusEl: HTMLElement | null = null;
  ...
  async render(options: RenderOptions = {}) {
    ...
    try {
      ...
      if (this.viewMode === "file") {
        ...
        const fileNoteRefs = this.getGroupNoteReferences(groups, fileInfo?.path ?? "");
      ...
      if (this.viewMode === "orphaned") {
        ...
        this.renderViewControls(contentEl, notes, this.orphanedNoteRefs, {
          ...
          fileSelectionGroups: orphanedFiles.map((file) =>
            this.getGroupNoteReferences(file.groups, file.path, file.SideNoteID)
          ),
      ...
      if (this.viewMode === "vault") {
        ...
        this.renderViewControls(contentEl, notes, this.vaultNoteRefs, {
          ...
          fileSelectionGroups: storedFiles.map((file) =>
            this.getGroupNoteReferences(file.groups, file.path, file.SideNoteID)
          ),
  ...
  private captureScrollState(anchorNoteKey?: string): SideNotesScrollState {
    ...
    return {
      ...
      viewContentTop: viewContentEl?.scrollTop ?? 0
    };
  }
  ...
  private findNoteCardEl(noteRefKey: string): HTMLElement | null {
    const cardEls = this.contentEl.querySelectorAll<HTMLElement>(".side-notes-card");
    return Array.from(cardEls).find((cardEl) => cardEl.dataset.noteRefKey === noteRefKey) ?? null;
  }
  ...
  private renderHeader(parent: HTMLElement, context: ParagraphContext | null, fileInfo: CurrentFileInfo | null) {
    ...
    setIcon(readingModeButtonEl, "book-open");
  ...
  private renderViewControls(parent: HTMLElement, notes: SideNote[], selectableNoteRefs: SideNoteReference[] = [], stats: SideNotesViewStats = { noteCount: selectableNoteRefs.length }) {
  ...
  private renderViewControlsContent(controlsEl: HTMLElement, notes: SideNote[], selectableNoteRefs: SideNoteReference[] = []) {
    ...
    if (this.selectedNotes.size === 0) {
      ...
      addIconButton(
        selectionControlsEl,
        "git-merge",
        canMergeSelectedNotes ? `Merge ${selectedCount} selected notes` : "Select at least two text notes in the same paragraph to merge",
        async () => {
          if (!this.canMergeSelectedNotes(selectedNoteRefs)) {
            new Notice("Select at least two text notes in the same paragraph to merge.");
            return;
          }
          ...
          if (mergedCount === 0) {
            new Notice("Could not merge the selected notes.");
            void this.render({ preserveScroll: true });
            return;
          }

          this.selectedNotes.clear();
          new Notice(`${mergedCount} notes merged into one.`);
        }
  ...
  private renderHeaderStats(selectableNoteRefs: SideNoteReference[], stats: SideNotesViewStats) {
    ...
    const selectedFileCount = this.getSelectedFileCount(stats.fileSelectionGroups ?? []);
    const orphanedText = (stats.orphanedFileCount ?? 0) > 0
      ? ` (${formatCount(stats.orphanedFileCount ?? 0, "orphaned file")})`
      : "";
  ...
  private getSelectedFileCount(fileSelectionGroups: SideNoteReference[][]): number {
    return fileSelectionGroups.filter((noteRefs) =>
      noteRefs.length > 0 && noteRefs.every((noteRef) => this.selectedNotes.has(this.getNoteReferenceKey(noteRef)))
    ).length;
  }
  ...
  private setNoteRefsSelection(noteRefs: SideNoteReference[], selected: boolean) {
    for (const noteRef of noteRefs) {
  ...
  private syncRenderedSelectionCheckboxes() {
    const checkboxEls = this.contentEl.querySelectorAll<HTMLInputElement>(".side-notes-note-checkbox");
    ...
    const bulkCheckboxEls = this.contentEl.querySelectorAll<HTMLInputElement>(".side-notes-bulk-checkbox");
  ...
  private setRenderedNoteSelection(noteRefKey: string, noteRef: SideNoteReference, checked: boolean, checkboxEl: HTMLInputElement) {
    const scrollState = this.captureScrollState();
    this.plugin.suppressContextRefreshBriefly();
  ...
  private renderSelectionCheckbox(parent: HTMLElement, noteRefs: SideNoteReference[], label: string) {
    const selectableNoteRefs = noteRefs.filter((noteRef) => !this.isNoteCut(noteRef));
    if (selectableNoteRefs.length === 0) {
      return;
    }
  ...
  private canMergeSelectedNotes(noteRefs: SideNoteReference[]): boolean {
    if (noteRefs.length < 2) {
      return false;
    }
    ...
    return noteRefs.every((noteRef) => {
      ...
      const note = this.plugin.getStoredFileNotesFromSource(noteRef.sourcePath, noteRef.sourceSideNoteId)
        ?.blocks[noteRef.blockId]
        ?.notes.find((item) => item.id === noteRef.noteId);
      return !!note && isTextNote(note);
    });
  }

  private getNoteSourceKey(noteRef: SideNoteReference): string {
    const sourceSideNoteId = noteRef.sourceSideNoteId ?? this.plugin.sideNotesData.sideNoteIds[noteRef.sourcePath];
  ...
  private saveComposerDraftFromDom() {
    const textarea = this.composerSlotEl?.querySelector<HTMLTextAreaElement>(".side-notes-textarea");
    if (!textarea) {
      return;
    }
  ...
  private renderComposer(parent: HTMLElement) {
    ...
    const addNoteButton = new ButtonComponent(composerActionsEl)
    ...
    const mediaButton = new ButtonComponent(composerActionsEl)
  ...
  private async addPastedMediaNote(media: ClipboardMediaFile) {
    ...
    new Notice(`${mediaLabel} note added.`);
    this.shouldFocusComposer = true;
    await this.render({ preserveScroll: true });
  }

  private showMediaNoteMenu(event: MouseEvent) {
    const menu = new Menu();
    menu.addItem((item) => {
      item
        .setTitle("Image")
  ...
  private renderAudioRecordingPanel(parent: HTMLElement) {
    ...
    this.audioRecordingStartButton = new ButtonComponent(actionsEl)
      .setButtonText("Start")
      .setIcon("mic")
      .setCta()
      .onClick(() => {
    ...
    this.audioRecordingStopButton = new ButtonComponent(actionsEl)
      .setButtonText("Stop and add")
      .setIcon("square")
      .onClick(() => this.stopAudioRecordingAndSave());
    this.audioRecordingCancelButton = new ButtonComponent(actionsEl)
  ...
  private async startAudioRecording() {
    ...
    if (!this.audioRecordingContext?.blockId) {
      const context = await this.plugin.ensureCurrentContextHasBlockId();
      if (!context?.blockId) {
        return;
      }
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      new Notice("Audio recording is not available in this Obsidian window.");
      return;
    }
  ...
  private stopAudioRecordingAndSave() {
    if (!this.audioRecordingMediaRecorder || this.audioRecordingMediaRecorder.state === "inactive") {
      return;
    }
  ...
  private async saveAudioRecording() {
    if (this.audioRecordingChunks.length === 0) {
      new Notice("No audio was recorded.");
      this.resetAudioRecordingState();
      this.renderComposerSlot();
      return;
    }
    ...
    if (!context?.blockId) {
      new Notice("Could not find the paragraph for this recording.");
      this.resetAudioRecordingState();
      this.renderComposerSlot();
      return;
    }

    const type = this.audioRecordingMediaRecorder?.mimeType || "audio/webm";
  ...
  private getAudioRecordingStatusText(): string {
    ...
    const fileName = this.audioRecordingContext?.fileName;
    return fileName ? `Ready to record for ${fileName}.` : "Ready to record.";
  }

  private captureDraftSelection(textarea: HTMLTextAreaElement) {
    this.draftSelectionStart = textarea.selectionStart;
    this.draftSelectionEnd = textarea.selectionEnd;
  }

  private recordDraftInput(textarea: HTMLTextAreaElement) {
  ...
  private undoDraft(textarea: HTMLTextAreaElement): boolean {
  ...
  private async renderNote(parent: HTMLElement, sourcePath: string, blockId: string, note: SideNote, sourceSideNoteId?: string, showJumpButton = false) {
    ...
    if (isEditing) {
      ...
      applyNoteEditorStyles(textarea, this.plugin.settings);
      enableMarkdownListContinuation(textarea);
      textarea.value = this.editDrafts.get(noteRefKey) ?? noteText;
      ...
      if (isExpanded) {
        if (isImage) {
          ...
          previewEl.querySelectorAll<HTMLElement>(
            "p, li, blockquote, table, th, td, a, span, em, strong, del, code"
          ).forEach((element) => {
      ...
      if (isImage && isExpanded) {
        const zoom = clampImageZoom(note.imageZoom ?? 1);
        const rotation = normalizeImageRotation(note.imageRotation ?? 0);
  ...
  private renderImageNotePreview(parent: HTMLElement, sourcePath: string, blockId: string, note: SideNote, sourceSideNoteId: string | undefined, noteRefKey: string) {
    ...
    const frameHeight = typeof note.imageFrameHeightPx === "number"
      ...
      : null;
    const frameScale = getImageFrameScale(
      note,
      frameHeight,
      previewEl.ownerDocument.defaultView?.innerHeight ?? window.innerHeight
    );
    const rotation = normalizeImageRotation(note.imageRotation ?? 0);
    const panX = zoom > 1 ? clampImagePanOffset(note.imagePanX ?? 0) : 0;
    const panY = zoom > 1 ? clampImagePanOffset(note.imagePanY ?? 0) : 0;
    frameEl.setCssProps({
    ...
    const imagePath = note.imagePath ?? "";
    ...
    frameEl.createEl("img", {
      ...
      attr: {
        alt: note.imageName ?? imageFile.name,
    ...
    previewEl.createDiv({
      ...
      text: `${note.imageName ?? imageFile.name} (${Math.round(zoom * frameScale * 100)}%${rotation ? `, ${rotation} deg` : ""})`
    });

    this.enableImageFrameGestures(frameEl, sourcePath, blockId, note, sourceSideNoteId, noteRefKey);
  }

  private renderAudioNotePreview(parent: HTMLElement, note: SideNote) {
    ...
    const updatePlayIcon = () => {
      playButtonEl.empty();
      setIcon(playButtonEl, audioEl.paused ? "play" : "pause");
    ...
    playButtonEl.addEventListener("click", (event) => {
      ...
      if (audioEl.paused) {
        void audioEl.play().catch((error) => {
          console.error(error);
          new Notice("Could not play this audio file.");
        });
      } else {
        audioEl.pause();
      }
    ...
    previewEl.createDiv({
      ...
      text: note.audioName ?? audioFile.name
    });
  }

  private renderVideoNotePreview(parent: HTMLElement, note: SideNote) {
    const previewEl = parent.createDiv({ cls: "side-notes-video-preview" });
    const videoPath = note.videoPath ?? "";
    ...
    previewEl.createDiv({
      ...
      text: note.videoName ?? videoFile.name
    });
  }

  private renderFileNotePreview(parent: HTMLElement, note: SideNote) {
    const previewEl = parent.createDiv({ cls: "side-notes-link-preview" });
    const title = note.fileName ?? getFileNoteDisplayPath(note);
    const storedFile = isStoredFileNote(note) ? this.plugin.app.vault.getAbstractFileByPath(note.filePath) : null;
    const missingStoredFile = isStoredFileNote(note) && !(storedFile instanceof TFile);

    const titleEl = previewEl.createDiv({ cls: "side-notes-link-title" });
    setIcon(titleEl.createSpan({ cls: "side-notes-link-icon" }), "file");
  ...
  private renderUrlNotePreview(parent: HTMLElement, note: SideNote) {
    const previewEl = parent.createDiv({ cls: "side-notes-link-preview" });
    const titleEl = previewEl.createDiv({ cls: "side-notes-link-title" });
    setIcon(titleEl.createSpan({ cls: "side-notes-link-icon" }), "link");
    titleEl.createSpan({ text: note.urlTitle ?? getUrlDisplayName(note.url ?? "") });
    previewEl.createDiv({
  ...
  private resizeContainingSidePanel(requestedFactor: number): number {
    const panelEl = this.containerEl.closest<HTMLElement>(
    ...
    const parentWidth = panelEl.parentElement?.getBoundingClientRect().width
      ?? this.containerEl.ownerDocument.defaultView?.innerWidth
      ?? currentWidth;
    const minimumPanelWidth = 240;
    const maximumPanelWidth = Math.max(minimumPanelWidth, Math.min(1600, parentWidth - 320));
    const nextWidth = Math.max(
      minimumPanelWidth,
      Math.min(maximumPanelWidth, Math.round(currentWidth * requestedFactor))
    );
    ...
    const panelWindow = this.containerEl.ownerDocument.defaultView ?? window;
  ...
  private enableImageFrameGestures(frameEl: HTMLElement, sourcePath: string, blockId: string, note: SideNote, sourceSideNoteId: string | undefined, noteRefKey: string) {
    const activePointers = new Map<number, PointerEvent>();
    const savedZoom = clampImageZoom(note.imageZoom ?? 1);
    const savedPanX = savedZoom > 1 ? clampImagePanOffset(note.imagePanX ?? 0) : 0;
    const savedPanY = savedZoom > 1 ? clampImagePanOffset(note.imagePanY ?? 0) : 0;
    const savedFrameHeight = typeof note.imageFrameHeightPx === "number"
      ? clampImageFrameHeight(note.imageFrameHeightPx)
      : null;
    const savedFrameScale = getImageFrameScale(
      note,
      savedFrameHeight,
      frameEl.ownerDocument.defaultView?.innerHeight ?? window.innerHeight
    );
    ...
    let pointerStartX = 0;
    let pointerStartY = 0;
    let touchTapStart: { pointerId: number; x: number; y: number; startedAt: number } | null = null;
    let lastTouchTapAt = 0;
    let lastTouchTapX = 0;
    let lastTouchTapY = 0;
    let wheelFrameSizeChanged = false;
    let wheelSaveTimerId: number | null = null;
    ...
    const setPreviewPan = (panX: number, panY: number) => {
      if (currentZoom <= 1) {
        currentPanX = 0;
        currentPanY = 0;
      } else {
    ...
    const scheduleWheelSave = (frameSizeChanged: boolean) => {
      ...
      wheelSaveTimerId = window.setTimeout(() => {
        ...
        const frameHeightToSave = wheelFrameSizeChanged ? currentFrameHeight ?? undefined : undefined;
        const frameScaleToSave = wheelFrameSizeChanged ? currentFrameScale : undefined;
        wheelFrameSizeChanged = false;
        void this.plugin.updateImageNoteViewport(
          blockId,
          note.id,
          currentZoom,
          frameHeightToSave,
    ...
    const resetImageView = () => {
      ...
      if (hasExternalZoom) {
        const frameHeight = currentFrameHeight ?? clampImageFrameHeight(frameEl.getBoundingClientRect().height);
    ...
    frameEl.addEventListener("pointermove", (event) => {
      ...
      activePointers.set(event.pointerId, event);
      if (
        touchTapStart?.pointerId === event.pointerId &&
    ...
    const isTouchDoubleTap = (event: PointerEvent): boolean => {
      if (event.pointerType !== "touch" || touchTapStart?.pointerId !== event.pointerId) {
        return false;
      }
  ...
  private renderNoteMoveButtons(parent: HTMLElement, sourcePath: string, blockId: string, note: SideNote, sourceSideNoteId?: string) {
    const siblingNotes = this.plugin.getStoredFileNotesFromSource(sourcePath, sourceSideNoteId)?.blocks[blockId]?.notes ?? [];
...
class ConfirmModal extends Modal {
  ...
  onOpen() {
    ...
    markDeleteButton(new ButtonComponent(actionsEl)
...
function promptTextInObsidian(app: App, title: string, placeholder: string): Promise<string | null> {
  return new Promise((resolve) => {
...
function getSideNotesSettingsFromPluginData(rawData: unknown): SideNotesSettings {
  ...
  return {
    ...
    noteOpenFontSizePx: getNumberValue(
      data.noteOpenFontSizePx,
      getNumberValue(data.notePreviewFontSizePx, noteFontSizePx)
    ),
...
function isImageNote(note: SideNote): note is SideNote & { imagePath: string } {
  return note.type === "image" && typeof note.imagePath === "string" && note.imagePath.length > 0;
}

function isAudioNote(note: SideNote): note is SideNote & { audioPath: string } {
  return note.type === "audio" && typeof note.audioPath === "string" && note.audioPath.length > 0;
}

function isVideoNote(note: SideNote): note is SideNote & { videoPath: string } {
  return note.type === "video" && typeof note.videoPath === "string" && note.videoPath.length > 0;
}

function isFileNote(note: SideNote): note is SideNote & { type: "file" } {
  return note.type === "file" && (
    (typeof note.filePath === "string" && note.filePath.length > 0) ||
    (typeof note.fileExternalPath === "string" && note.fileExternalPath.length > 0)
  );
}

function isStoredFileNote(note: SideNote): note is SideNote & { type: "file"; filePath: string } {
  return note.type === "file" && typeof note.filePath === "string" && note.filePath.length > 0;
}

function isExternalFileNote(note: SideNote): note is SideNote & { type: "file"; fileExternalPath: string } {
  return note.type === "file" && typeof note.fileExternalPath === "string" && note.fileExternalPath.length > 0;
}

function isUrlNote(note: SideNote): note is SideNote & { type: "url"; url: string } {
  return note.type === "url" && typeof note.url === "string" && note.url.length > 0;
}

function isTextNote(note: SideNote): boolean {
  return !isImageNote(note) && !isAudioNote(note) && !isVideoNote(note) && !isFileNote(note) && !isUrlNote(note);
}

function getSideNoteMediaPath(note: SideNote): string | null {
  if (isImageNote(note)) {
...
function getSideNoteMediaPathsFromBlocks(blocks: Record<string, BlockNotes>): Set<string> {
...
function remapSideNoteMediaPaths(blocks: Record<string, BlockNotes>, pathMap: Map<string, string>) {
  if (pathMap.size === 0) {
    return;
  }
...
function getRemappedSideNoteMediaPath(path: string, pathMap: Map<string, string>): string {
  const sanitizedPath = sanitizeOptionalVaultPath(path);
  if (!sanitizedPath) {
    return path;
  }

  return pathMap.get(sanitizedPath) ?? path;
}

function getSideNoteSummaryText(note: SideNote): string {
  const noteText = note.text;
  if (isImageNote(note)) {
    return `Image: ${note.imageName ?? getLeafFileName(note.imagePath ?? "")}`;
  }

  if (isAudioNote(note)) {
    return `Audio: ${note.audioName ?? getLeafFileName(note.audioPath ?? "")}`;
  }

  if (isVideoNote(note)) {
    return `Video: ${note.videoName ?? getLeafFileName(note.videoPath ?? "")}`;
  }

  if (isFileNote(note)) {
    return `File: ${note.fileName ?? getFileNoteDisplayPath(note)}`;
  }

  if (isUrlNote(note)) {
    return `URL: ${note.urlTitle ?? getUrlDisplayName(note.url)}`;
  }
...
function clampImageFrameHeight(height: number): number {
  return Math.max(160, Math.min(1600, Math.round(height)));
}

function clampImageFrameScale(scale: number): number {
  return Math.max(0.25, Math.min(4, Math.round(scale * 1000) / 1000));
}

function getImageFrameScale(note: SideNote, frameHeight: number | null, viewportHeight: number): number {
...
function clampImagePanOffset(offset: number): number {
  return Math.max(-5000, Math.min(5000, Math.round(offset)));
}

function normalizeImageRotation(rotation: number): number {
  return ((Math.round(rotation / 90) * 90) % 360 + 360) % 360;
}

interface ClipboardMediaFile {
  kind: "image" | "audio" | "video";
  file: File;
}

function getClipboardMediaFile(event: ClipboardEvent): ClipboardMediaFile | null {
...
function getClipboardMediaKind(fileName: string, mimeType: string): ClipboardMediaFile["kind"] | null {
...
function normalizeClipboardMediaFile(
  file: File,
  mimeType: string,
  kind: ClipboardMediaFile["kind"]
): File {
...
function isAcceptedImageFile(fileName: string, mimeType: string): boolean {
  return mimeType.startsWith("image/") || /\.(?:avif|bmp|gif|ico|jpe?g|png|svg|webp)$/i.test(fileName);
}

function getSafeImageFileName(fileName: string, mimeType: string): string {
...
function getImageExtension(fileName: string, mimeType: string): string {
  ...
  const subtype = mimeType.match(/^image\/([A-Za-z0-9.+-]+)$/)?.[1];
  return subtype ? `.${subtype.replace(/[^A-Za-z0-9]/g, "")}` : ".png";
}

function isAcceptedAudioFile(fileName: string, mimeType: string): boolean {
  return mimeType.startsWith("audio/") || /\.(?:aac|aif|aiff|flac|m4a|mp3|oga|ogg|opus|wav|webm|wma)$/i.test(fileName);
}

function isAcceptedVideoFile(fileName: string, mimeType: string): boolean {
  return mimeType.startsWith("video/") || /\.(?:avi|m4v|mkv|mov|mp4|mpeg|mpg|ogv|webm|wmv)$/i.test(fileName);
}

function getSafeAudioFileName(fileName: string, mimeType: string): string {
...
function getAudioExtension(fileName: string, mimeType: string): string {
  ...
  const subtype = mimeType.match(/^audio\/([A-Za-z0-9.+-]+)$/)?.[1];
  return subtype ? `.${subtype.replace(/[^A-Za-z0-9]/g, "")}` : ".mp3";
}

function getSafeVideoFileName(fileName: string, mimeType: string): string {
...
function getVideoExtension(fileName: string, mimeType: string): string {
  ...
  const subtype = mimeType.match(/^video\/([A-Za-z0-9.+-]+)$/)?.[1];
  return subtype ? `.${subtype.replace(/[^A-Za-z0-9]/g, "")}` : ".mp4";
}

function getSafeGenericFileName(fileName: string): string {
...
function getFileNoteDisplayPath(note: SideNote): string {
  if (isStoredFileNote(note)) {
    return note.filePath;
  }
...
async function openFilePathExternally(path: string, fallbackUrl?: string): Promise<void> {
  const url = pathToFileUrl(path) ?? fallbackUrl;
  if (!url) {
...
function getPreferredAudioRecordingMimeType(): string {
...
function addToolbarIconButton(parent: HTMLElement, label: string, icon: string, onClick: (event: MouseEvent) => void) {
  const button = parent.createEl("button", {
    attr: {
      "aria-label": label,
      title: label
...
function markDeleteButton(button: ButtonComponent): ButtonComponent {
  button.buttonEl.addClass("side-notes-delete-button");
  return button;
}

function markClearSelectionButton(button: ButtonComponent): ButtonComponent {
...
async function copyRichContentToClipboard(plainText: string, html: string, nativeImage: Blob | null = null): Promise<void> {
  if (navigator.clipboard?.write && typeof ClipboardItem !== "undefined") {
    const clipboardRepresentations: Record<string, Blob> = {
...
async function getSideNoteImageClipboardBlob(app: App, note: SideNote & { imagePath: string }): Promise<Blob | null> {
...
async function buildSideNotesClipboardHtml(app: App, notes: SideNote[]): Promise<string> {
...
async function getNoteRichCopyHtml(app: App, note: SideNote): Promise<string> {
  if (isImageNote(note)) {
    const name = note.imageName ?? getLeafFileName(note.imagePath);
  ...
  if (isAudioNote(note)) {
    const name = note.audioName ?? getLeafFileName(note.audioPath);
  ...
  if (isVideoNote(note)) {
    const name = note.videoName ?? getLeafFileName(note.videoPath);
  ...
  if (isUrlNote(note)) {
    const label = note.urlTitle ?? getUrlDisplayName(note.url);
...
async function getSideNoteMediaDataUrl(
  app: App,
  path: string,
...
function setEditTextareaInitialHeight(textarea: HTMLTextAreaElement) {
  window.requestAnimationFrame(() => {
...
function getElementTopRelativeTo(containerEl: HTMLElement, elementEl: HTMLElement): number {
  return elementEl.getBoundingClientRect().top - containerEl.getBoundingClientRect().top;
}

function restoreElementTopRelativeTo(containerEl: HTMLElement, elementEl: HTMLElement, previousTop: number | undefined) {
...
function getDatasetList(value: string | undefined): string[] {
  return value ? value.split("\n").filter((item) => item.length > 0) : [];
}

function joinMergedNoteTexts(texts: string[]): string {
  return texts
...
function getNoteExportMarkdown(note: SideNote): string {
  const noteText = note.text;
  if (isImageNote(note)) {
    return `![[${note.imagePath}]]`;
  }
  ...
  if (isExternalFileNote(note)) {
    const label = note.fileName ?? getFileNoteDisplayPath(note);
    return `[${escapeMarkdownLinkText(label)}](${pathToFileUrl(note.fileExternalPath) ?? note.fileExternalPath})`;
  }

  if (isUrlNote(note)) {
    return `[${escapeMarkdownLinkText(note.urlTitle ?? getUrlDisplayName(note.url))}](${note.url})`;
  }

  return noteText;
}

function getNotePlainCopyText(note: SideNote): string {
  const noteText = note.text;
  ...
  if (isFileNote(note)) {
    return `[File: ${note.fileName ?? getFileNoteDisplayPath(note)}] ${getFileNoteDisplayPath(note)}`;
  }

  if (isUrlNote(note)) {
    return `[URL: ${note.urlTitle ?? getUrlDisplayName(note.url)}] ${note.url}`;
  }
...
function applyBulletList(textarea: HTMLTextAreaElement) {
  applyLinePrefixes(textarea, () => "- ");
}

function applyTaskList(textarea: HTMLTextAreaElement) {
  applyLinePrefixes(textarea, () => "- [ ] ");
}

function applyNumberedList(textarea: HTMLTextAreaElement) {
...
function outdentSelectedLines(textarea: HTMLTextAreaElement) {
...
function transformSelectedLines(textarea: HTMLTextAreaElement, transformLine: (line: string) => string) {
  const start = textarea.selectionStart;
  const end = textarea.selectionEnd;
  const value = textarea.value;
  const lineStart = value.lastIndexOf("\n", start - 1) + 1;
...
function applyLinePrefixes(textarea: HTMLTextAreaElement, getPrefix: (prefixedLineIndex: number) => string) {
  const start = textarea.selectionStart;
  const end = textarea.selectionEnd;
  const value = textarea.value;
  const lineStart = value.lastIndexOf("\n", start - 1) + 1;
...
function getLineWithoutListMarker(line: string): { indent: string; content: string } {
